...
export default function ToolDetailPage() {
  ...
  if (!tool) return (
    <div className="pt-32 flex flex-col items-center gap-4">
      <p className="font-display font-bold text-2xl" style={{ color:'var(--os-text3)' }}>Tool not found</p>
      <button onClick={() => navigate('/tools')} className="px-5 py-2.5 rounded-xl text-sm" style={{ background:'var(--os-accent)', color:'#fff' }}>Back to tools</button>
    </div>
  );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}>
          ...
          <div className="rounded-3xl overflow-hidden mb-6"
            style={{ background:`radial-gradient(ellipse at 50% 0%, ${c}15 0%, transparent 60%), var(--os-card)`, border:'1px solid rgba(255,255,255,0.07)' }}>
            <div className="p-8 pb-6">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h1 className="font-display font-bold text-2xl" style={{ color:'var(--os-text)' }}>{tool.name}</h1>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => togglePin(tool.id)}
                    ...
                    style={{ background: pin ? c+'20' : 'rgba(255,255,255,0.06)', color: pin ? c : 'var(--os-text2)', border:`1px solid ${pin ? c+'40' : 'rgba(255,255,255,0.08)'}` }}>
                    <Pin size={12} /> {pin ? 'Pinned' : 'Pin'}
                  </button>
                  <button onClick={() => toggleFavorite(tool.id)}
                    className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-medium transition-all"
                    style={{ background: fav ? 'rgba(239,68,68,0.12)' : 'rgba(255,255,255,0.06)', color: fav ? '#ef4444' : 'var(--os-text2)', border:`1px solid ${fav ? '#ef444440' : 'rgba(255,255,255,0.08)'}` }}>
              ...
              <p className="mt-6 text-sm leading-relaxed max-w-2xl" style={{ color:'var(--os-text2)' }}>{tool.description}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Links */}
            <div className="rounded-2xl p-5" style={{ background:'var(--os-card)', border:'1px solid rgba(255,255,255,0.06)' }}>
              <p className="text-xs font-mono tracking-widest mb-4" style={{ color:'var(--os-text3)' }}>LINKS</p>
              <div className="space-y-2">
                {[['GitHub', tool.github, Github], ['Docs', tool.docs, ExternalLink], ['Demo', tool.demo, ExternalLink]].filter(([,v]) => v).map(([label, url, Icon]) => (
                  <a key={label} href={url} target="_blank" rel="noopener noreferrer"
                    ...
                    {label}
                    <ExternalLink size={10} style={{ color:'var(--os-text3)', marginLeft:'auto' }} />
                  </a>
                ))}
                {!tool.github && !tool.docs && !tool.demo && (
                  <p className="text-xs" style={{ color:'var(--os-text3)' }}>No links available</p>
                )}
              </div>
            </div>

            {/* Changelog */}
            <div className="md:col-span-2 rounded-2xl p-5" style={{ background:'var(--os-card)', border:'1px solid rgba(255,255,255,0.06)' }}>
              <p className="text-xs font-mono tracking-widest mb-4" style={{ color:'var(--os-text3)' }}>CHANGELOG</p>
              <div className="space-y-4">
                {(tool.changelog || []).map((entry, i) => (
                  <div key={i} className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div className="w-2 h-2 rounded-full mt-1.5 shrink-0" style={{ background: i===0 ? c : 'var(--os-text3)' }} />
                      {i < (tool.changelog||[]).length - 1 && <div className="w-px flex-1 mt-1" style={{ background:'rgba(255,255,255,0.08)' }} />}
                    </div>
                    <div className="pb-3">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs font-mono font-semibold" style={{ color: i===0 ? c : 'var(--os-text2)' }}>v{entry.version}</span>
                        <span className="text-[10px] font-mono" style={{ color:'var(--os-text3)' }}>{entry.date}</span>
                      </div>
                      <ul className="space-y-0.5">
                        {entry.changes.map((ch, j) => (
                          <li key={j} className="text-xs" style={{ color:'var(--os-text2)' }}>· {ch}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
