    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between p-5 border-b sticky top-0" style={{ borderColor: 'rgba(0,0,0,0.05)', background: 'var(--card)' }}>
              <h2 className="text-base font-bold" style={{ color: 'var(--text)' }}>Compare Tools</h2>
            ...
            <div className="overflow-y-auto" style={{ maxHeight: 'calc(85vh - 60px)' }}>
              {tools.length === 0 ? (
                <div className="p-10 text-center">
                  <p className="text-sm mb-4" style={{ color: 'var(--text3)' }}>Select up to 4 tools to compare</p>
                  <div className="flex flex-wrap justify-center gap-2 max-w-xl mx-auto">
                    {availableTools.slice(0, 8).map(t => {
                      const c = CAT_COLORS[t.category] || 'var(--accent)';
                      return (
                ...
                <div className="p-5">
                      ...
                      return (
                        <div key={tool.id} className="text-center">
                          <div className="flex items-center justify-between mb-2">
                            <div />
                            <button onClick={() => removeFromCompare(tool.id)} className="w-6 h-6 rounded-lg flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.05)' }}>
                              <X size={11} style={{ color: 'var(--text3)' }} />
                            </button>
                          </div>
                          <div className="w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-2" style={{ background: c + '18' }}>
                            <Icon size={18} style={{ color: c }} />
                          </div>
                          <p className="text-sm font-bold" style={{ color: 'var(--text)' }}>{tool.name}</p>
                  ...
                  <div className="space-y-1">
                    {FEATURE_FIELDS.map(field => (
                      <div key={field.key} className="grid gap-4 py-3 px-3 rounded-lg" style={{ gridTemplateColumns: `140px repeat(${tools.length}, 1fr)`, background: 'var(--bg)' }}>
                        <span className="text-xs font-medium" style={{ color: 'var(--text2)' }}>{field.label}</span>
                        {tools.map(tool => (
                          <div key={tool.id} className="text-center">
                  ...
                  <div className="mt-4">
                    <p className="text-xs font-medium mb-3" style={{ color: 'var(--text2)' }}>Description</p>
                    <div className="grid gap-4" style={{ gridTemplateColumns: `140px repeat(${tools.length}, 1fr)` }}>
                      <div />
                      {tools.map(tool => (
                        <div key={tool.id} className="text-xs leading-relaxed" style={{ color: 'var(--text3)' }}>
                          {tool.description}
                        </div>
                      ))}
                    </div>
                  ...
                  {/* Tags comparison */}
                  <div className="mt-4">
                    <p className="text-xs font-medium mb-3" style={{ color: 'var(--text2)' }}>Tags</p>
                    <div className="grid gap-4" style={{ gridTemplateColumns: `140px repeat(${tools.length}, 1fr)` }}>
                      <div />
                      {tools.map(tool => (
                        <div key={tool.id} className="flex flex-wrap gap-1">
                          {(tool.tags || []).slice(0, 5).map(tag => (
                            <span key={tag} className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={{ background: 'var(--bg)', color: 'var(--text3)' }}>
                              {tag}
                            </span>
                          ))}
                        </div>
