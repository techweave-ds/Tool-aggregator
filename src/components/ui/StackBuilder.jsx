          ...
          >
            ...
            <div className="flex items-center justify-between p-5 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold" style={{ color: 'var(--os-text)' }}>AI Stack Builder</h2>
                  <p className="text-xs" style={{ color: 'var(--os-text3)' }}>What are you trying to build?</p>
                </div>
              </div>
            ...
            <div className="p-5 space-y-5">
              {!submitted ? (
                <>
                  ...
                  <form onSubmit={handleSubmit}>
                    <div className="flex gap-2">
                      <input
                        ...
                        style={{ background: 'var(--os-card2)', border: '1px solid rgba(255,255,255,0.08)', color: 'var(--os-text)' }}
                        autoFocus
                      />
                      <button type="submit"
                  ...
                  {/* Presets */}
                  <div>
                    <p className="text-xs font-mono mb-3" style={{ color: 'var(--os-text3)' }}>Quick start:</p>
                      ...
                      {PRESETS.map(p => {
                        ...
                        return (
                          <button key={p.id} onClick={() => handlePreset(p)}
                            ...
                            <span className="text-xs font-medium leading-tight" style={{ color: 'var(--os-text)' }}>{p.label}</span>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  {savedStacks.length > 0 && (
                    <div>
                      <p className="text-xs font-mono mb-3" style={{ color: 'var(--os-text3)' }}>Saved stacks:</p>
                        ...
                        {savedStacks.slice(-3).reverse().map(s => {
                          ...
                          return (
                            <div key={s.id} className="flex items-center justify-between p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                              <div>
                                <p className="text-sm font-medium" style={{ color: 'var(--os-text)' }}>{s.name}</p>
                                <p className="text-[10px] font-mono mt-0.5" style={{ color: 'var(--os-text3)' }}>
                                  {tools.map(t => t?.name).join(' · ')}
                                </p>
                              </div>
                              <span className="text-[10px] font-mono px-2 py-1 rounded" style={{ background: 'rgba(99,102,241,0.1)', color: 'var(--os-accent)' }}>
                                {tools.length} tools
                              </span>
                            </div>
                          );
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-mono px-3 py-1.5 rounded-full" style={{ background: 'rgba(99,102,241,0.1)', color: 'var(--os-accent)' }}>
                      "{query}"
                    </span>
                    <button onClick={handleReset} className="text-xs" style={{ color: 'var(--os-text3)' }}>Try another →</button>
                  </div>
                  ...
                  {generatedStack && (
                    ...
                    >
                      ...
                      <div className="p-4 rounded-xl" style={{ background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.15)' }}>
                        <h3 className="text-sm font-bold mb-1" style={{ color: 'var(--os-text)' }}>Recommended Stack</h3>
                        <p className="text-xs" style={{ color: 'var(--os-text2)' }}>{generatedStack.description}</p>
                      </div>
                          ...
                          return (
                            <div key={tool.id}
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  <p className="text-sm font-semibold" style={{ color: 'var(--os-text)' }}>{tool.name}</p>
                                  <span className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={{ background: c + '15', color: c }}>{tool.category}</span>
                                </div>
                                <p className="text-xs mt-0.5" style={{ color: 'var(--os-text3)' }}>{tool.description}</p>
                              </div>
                            </div>
                          );
                        })}
                      ...
                      {generatedStack.explanation && (
                        <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                          <p className="text-xs leading-relaxed" style={{ color: 'var(--os-text2)' }}>
                            <span className="font-semibold" style={{ color: 'var(--os-text)' }}>Why this stack: </span>
                            {generatedStack.explanation}
                          </p>
                        </div>
                      )}
                      ...
                      <div className="flex gap-2 pt-2">
                        <button onClick={handleSave}
                          ...
                          style={{ background: saved ? 'rgba(34,197,94,0.15)' : 'var(--os-accent)', color: saved ? '#22c55e' : '#fff', border: saved ? '1px solid rgba(34,197,94,0.3)' : 'none' }}>
