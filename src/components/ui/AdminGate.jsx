  ...
  return (
    <div className="flex items-center justify-center" style={{ minHeight:'60vh' }}>
      <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}
        ...
        <div className="flex items-center gap-3 px-6 py-5" style={{ borderBottom:'1px solid rgba(0,0,0,0.05)' }}>
          ...
          <div>
            ...
            <p className="text-[11px] font-mono mt-0.5" style={{ color:'var(--text3)' }}>Admin access required</p>
          </div>
        </div>
        <form onSubmit={tryUnlock} className="p-6 space-y-4">
          <div>
            <label className="block text-[10px] font-mono mb-2" style={{ color:'var(--text3)' }}>PIN</label>
            <motion.div animate={shake ? { x:[-8,8,-6,6,-3,3,0] } : {}} transition={{ duration:0.4 }} className="relative">
              <input type={show?'text':'password'} value={pin}
                ...
                style={{ background:'var(--bg)', border:`1px solid ${err ? '#ef4444' : 'rgba(0,0,0,0.08)'}`, letterSpacing:'0.3em' }}
              />
          ...
          <p className="text-[10px] text-center font-mono" style={{ color:'var(--text3)' }}>
