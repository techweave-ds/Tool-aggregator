      ...
      {open && (
        <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
          ...
          <motion.div
            ...
            style={{ background:'var(--os-card)', border:'1px solid rgba(99,102,241,0.25)' }}
          >
            <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
              <h2 className="font-display font-bold text-sm" style={{ color:'var(--os-text)' }}>Add Tool</h2>
              <button onClick={onClose} style={{ color:'var(--os-text3)' }}><X size={15} /></button>
            </div>
            <form onSubmit={submit} className="p-5 space-y-3.5 max-h-[70vh] overflow-y-auto">
                ...
                <div key={key}>
                  ...
                  {key === 'description'
                    ? <textarea value={f[key]} onChange={e => u(key,e.target.value)} rows={2} placeholder={ph}
                        className="w-full px-3 py-2 text-sm rounded-lg resize-none" style={{ background:'rgba(255,255,255,0.05)' }} />
                    : <input value={f[key]} onChange={e => u(key,e.target.value)} placeholder={ph} required={req}
                        className="w-full px-3 py-2 text-sm rounded-lg" style={{ background:'rgba(255,255,255,0.05)' }} />
                  }
                </div>
              ))}
              ...
              <div>
                <label className="block text-[10px] font-mono mb-1.5" style={{ color:'var(--os-text3)' }}>URL</label>
                <input value={f.url} onChange={e => u('url',e.target.value)} placeholder="https://…"
                  className="w-full px-3 py-2 text-sm rounded-lg" style={{ background:'rgba(255,255,255,0.05)' }} />
              </div>
              <div>
                <label className="block text-[10px] font-mono mb-1.5" style={{ color:'var(--os-text3)' }}>TAGS (comma separated)</label>
              ...
              <div className="flex gap-2 pt-1">
                <button type="button" onClick={onClose}
                  ...
                  style={{ border:'1px solid rgba(255,255,255,0.1)', color:'var(--os-text2)' }}>Cancel</button>
                <button type="submit"
                  className="flex-1 py-2.5 rounded-xl text-sm font-semibold"
                  style={{ background:'var(--os-accent)', color:'#fff' }}>Add Tool</button>
              </div>
            </form>
          </motion.div>
        </motion.div>
